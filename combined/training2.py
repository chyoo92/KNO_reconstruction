...
def main_one_gpu(args):
    ...
    if args.transfer_learning == 0:
        model = perceiver_i2(fea = args.fea, \
                        cla = args.cla, \
...
def main_multi_gpu(rank,args):
    ...
    if args.transfer_learning == 0:
        ...
        optm.load_state_dict(checkpoint['optimizer_state_dict'])
        scheduler = CyclicLR(optm, base_lr=0.001, max_lr=0.01, step_size_up= 20,step_size_down=None,mode='triangular')
    ...
    print(len(trnLoader),'eeeeeeeeeeeeeeeeeee')
    if args.transfer_learning == 0:
        optm = torch.optim.SGD(model.parameters(), lr=0.001,momentum=0.9)
        # optm = torch.optim.Adam(model.parameters(), lr=config['training']['learningRate'])
        scheduler = torch.optim.lr_scheduler.OneCycleLR(optm, max_lr=0.1,steps_per_epoch=len(trnLoader),epochs=200)
