...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def forward(self, query, key_value, mask):
        ...
        # print(energy[0,15,90:99,24728:24730])
        # print(torch.where(energy>0))

        mask = mask.unsqueeze(1)
        mask = mask.expand(-1, energy.shape[2], -1)
        mask = mask.unsqueeze(1)
        mask = mask.expand(-1, self.n_heads, -1, -1)
        mask = mask.reshape(batch_size, self.n_heads, energy.shape[2], energy.shape[3])

        energy = energy.masked_fill(mask==0, float('-inf'))
        # print(energy[0,15,90:99,24728:24730], 'mask energy')
        # stop
