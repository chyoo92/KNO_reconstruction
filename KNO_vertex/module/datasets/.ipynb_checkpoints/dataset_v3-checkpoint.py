...
class dataset_v3(PyGDataset):
    def __init__(self, **kwargs):
        super(dataset_v3, self).__init__(None, transform=None, pre_transform=None)
        self.isLoaded = False
    ...
    def initialize(self, output):
        ...
        for i, fName in tqdm(enumerate(self.sampleInfo['fileName'])):
            ...
            for j in range(nEvents):
                if (f[j].x.shape[0]>1000)&(np.linalg.norm(aaa-np.array(f[j].y))<3500):
